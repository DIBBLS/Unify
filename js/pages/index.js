...
const _rl = {};
function rateLimit(key, max=5, ms=60000) {
  const now = Date.now();
  if (!_rl[key]) _rl[key] = [];
  _rl[key] = _rl[key].filter(t => now - t < ms);
...
function postCardHTML(p){
  const init=sanitise((p.author||'U')[0].toUpperCase());
  ...
  return `<div class="post-card" id="pc-${p.id}">
    <div class="pc-meta">
      ...
      <div><div class="pc-name">${sanitise(p.author)||'You'}</div><div class="pc-info">${[sanitise(p.dept),sanitise(p.university)].filter(Boolean).join(' · ')}</div></div>
      <div class="pc-time">${timeAgo(ts)}</div>
    </div>
    ${p.tag?`<span class="pc-tag">${sanitise(p.tag)}</span>`:''}
    <div class="pc-content">${sanitise(p.content)}</div>
    ${collabHTML}
    ...
    <div class="comments-wrap" id="cw-${p.id}">
      <div class="comment-list" id="cl-${p.id}">
        ${(p.comments||[]).map(c=>`<div class="comment"><div class="cm-av">${(c.author||'?')[0].toUpperCase()}</div><div class="cm-bubble"><div class="cm-author">${sanitise(c.author)}</div><div class="cm-text">${sanitise(c.text)}</div></div></div>`).join('')}
      </div>
...
function renderWorks(){
  ...
  list.innerHTML=works.map(w=>`<div class="work-card">
    <div class="wc-top"><div class="wc-title">${sanitise(w.name)}</div><span class="wc-status ${sc[w.status]||'wc-idea'}">${sl[w.status]||w.status}</span></div>
    <div class="wc-desc">${sanitise(w.desc)}</div>
...
window.submitPost=async function(){
  ...
  if(!content)return;
  if(!rateLimit('post-'+(currentUser?.uid||'x'),5,60000)){showToast('Max 5 posts per minute.');return;}
  const name=profile.firstName||currentUser.displayName||currentUser.email?.split('@')[0]||'Builder';
...
window.addComment=async function(id){
  ...
  if(!rateLimit('cm-'+(currentUser?.uid||'x'),10,60000)){showToast('Slow down.');return;}
...
window.saveProfile=async function(){
  ...
  if(bio.length>500){showToast('Bio must be under 500 characters.');return;}
  const headline=document.getElementById('dHeadline').value.trim();
  if(headline.length>120){showToast('Headline must be under 120 characters.');return;}
